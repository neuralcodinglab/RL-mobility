...
JR:
Taken from:
https://github.com/scikit-image/scikit-image/blob/376d30f5ba83e98d2745622d9d4951914e0c91f7/skimage/feature/_canny.py 
adapted to work in differentiable programming pipeline (and using only single threshold)
------
...
#JR: for the differentiable canny module
import torch
...
def _canny(image, sigma=1., low_threshold=None, high_threshold=None,
...
#     return output_mask                  #Only change: return the magnitude and local maxima
    return magnitude, local_maxima


class DifferentiableCannyModule(nn.Module):
    """ Wrapper to use the scikit-image Canny function in differentiable 
    programming pipeline, with adaptive (single) threshold.

    Usage: initialize with sigma and start threshold. 
    Takes numpy img, performs CED and returns in pytorch img format. 
    
    if adaptive_threshold == True, the threshold parameter can be optimized using backprop. 

""" 
    def __init__(self, sigma=1, threshold=0.47, adaptive_threshold=False, device='cuda:0'):
        super(DifferentiableCannyModule, self).__init__()
    ...
    def forward(self,image):
        ...
        magnitude, local_maxima = _canny(image, self.sigma)
        thin_edges = torch.from_numpy(magnitude * local_maxima)
        thin_edges = thin_edges.to(self.device).float().view(1,1,*image.shape)
