...
#             frame = img_processing(state_raw).to(agent.device)
            frame = cv2.cvtColor(state_raw['colors'], cv2.COLOR_BGR2GRAY)
            stack = frame_stack.update_with(frame)
            observation = np.expand_dims(np.stack(stack),0)
...
def train(agent, environment, img_processing, optimizer_dqn, cfg, optimizer_canny=None, scheduler_canny=None):
    ...
    optimizer_dqn.optimization_count = 0
    target_net_update_count = 0

    for episode in range(cfg['max_episodes']):
        
        if cfg['adaptive_threshold']:
            edge_threshold = agent.canny_layer.threshold.item()
        else:
        ...
        if optimizer_dqn.optimization_count > cfg['max_optim_steps']:
            break


        # Target net is updated once in a few steps (double Q-learning)
        if optimizer_dqn.optimization_count / cfg['target_update'] >= target_net_update_count:  #steps
...
#                 next_state = torch.cat([next_state,fwd_steps*torch.ones(1,1,cfg['imsize'],cfg['imsize'],device=cfg['device'])], dim=1)/cfg['n_target_steps']
            ...
            if len(agent.memory) > cfg['batch_size']: # TODO: should be cfg['replay_start_size']:
                ...
                for _ in range(cfg['optimizations_per_step']):
                    ...
                    # Optimize the model
                    if optimizer_canny is not None:
                        optimizer_canny.zero_grad()
                    ...
                    optimizer_dqn.step()
                    optimizer_dqn.optimization_count += 1
                    
                    if optimizer_canny is not None: # and optimizer_dqn.optimization_count > cfg['max_optim_steps']//2:
                        optimizer_canny.step()
...
#         img_processing = imgproc.ImageProcessor(**cfg)
        optimizer_canny = None
        if cfg['adaptive_threshold']:
            print("Using adaptive edge threshold") # TODO: this print statement can be removed
            optimizer_canny = optim.Adam(agent.canny_layer.parameters(), lr = cfg['lr_canny'])
            scheduler_canny = optim.lr_scheduler.LinearLR(optimizer_canny,1,.001, cfg['max_optim_steps']/2)
        optimizer_dqn = optim.Adam(agent.policy_net.parameters(), lr = cfg['lr_dqn'])
        ...
        train(agent, environment, img_processing, optimizer_dqn, cfg, optimizer_canny)
